...
public class MeshData
{
    ...
    public MeshData(Terrain terrain, int chunkSize, float treshold = 0.01f)
    {
        ChunkSize = chunkSize;
        ...
        for (int chunkX = 0; chunkX < width; chunkX++)
        {
            for (int chunkY = 0; chunkY < height; chunkY++)
            {
                ...
                for (int x = 0; x < chunkSize; x++)
                {
                    ...
                    {
                        data.Vertices[i] = new Vector3(topLeftX + chunkOffsetX + x, terrain.HeightMap[x + chunkOffsetX, y + chunkOffsetY], topLeftZ - chunkOffsetY - y);
                        data.UVs[i] = new Vector2(x / (float)terrain.Width - chunkOffsetX, y / (float)terrain.Height - chunkOffsetY);

                        if (terrain.HeightMap[x + chunkOffsetX, y + chunkOffsetY] >= treshold) {
                            if (x < chunkSize - 1 && y < chunkSize - 1) {
                                // data.AddTriangle(i, i + chunkSize + 1, i + chunkSize);
                                data.AddTriangle(i + chunkSize, i + chunkSize + 1, i);
                                // data.AddTriangle(i, i + 1, i + chunkSize + 1);
                                data.AddTriangle(i + chunkSize + 1, i + 1, i);
                            }
                        }
